...
def handle_put(key):
  ...
  # broadcast
  responses = asyncio.run(broadcast(request.method, f'/internal_shard/kvs/internal/replicate/{key}', key, globals.local_clocks, val=val, source=globals.address, node_id=globals.node_id)) # change data_clocks[key] to the sending_vc
...
def get(key):
    ...
    while(compare(globals.local_clocks, key, causal_metadata.get(key, [0]*len(globals.current_view)))<=0):
        ...
        responses = asyncio.run(broadcast("GET", f"/internal_shard/kvs/internal/replicate/{key}", key, causal_metadata[key], val=None, node_id=globals.node_id, source= globals.address))
